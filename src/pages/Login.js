...
export default function Login () {
  ...
  const {handleChange, handleSubmit, chkBlankFormLogin, chkFormErrors, noBlanks, isValidUser, isValidPassword, values, formErrors} = useForm (submit);
...
//  const {user, password} = values
  ...
  function submit() {
    if (chkBlankFormLogin ()){
      ...
      setAlertMessage(prevState => ( {...prevState, severity:"success", title: "Error en inicio de sesión", message:"Nombre de usuario y contraseña inválidos. Favor intentar nuevamente."}));
        setIsAlertOpen(true);
        history.push('/sponsor');  
      }
  } 
          ...
          <Paper elevation={6} spacing={2} className={classes.paperStyle}>
            ...
            <Grid item xs={12} md={12} spacing={1}> 
              <TextField id="user" label="Nombre de usuario *" 
                ...
                name="user" value={values.user} onChange={ (e) => handleChange (e,[noBlanks])}
            ...
            <Grid item xs={12} md={9} spacing={1}> 
              <TextField id="password" label="Contraseña *"
                 ...
                 name="password" value={values.password} onChange={(e) => handleChange (e,[noBlanks])}
